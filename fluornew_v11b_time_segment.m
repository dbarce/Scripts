...
BW=imerode(BW,[se90,se0]);
 BW=imclearborder(BW);
[labi,num]=bwlabel(BW);
res=regionprops(labi,'Area','PixelList'); 
%     
...
% figure('name',strcat(int2str(j),' Segment'));imshow(BW);%Combined
% %hold on
% pause
% end
...
for jk=1:num
   ...
   if marea>1000    
      ...
      long=size(vec,1);
      veco_GFP=zeros(long,1);
      veco_TOM=zeros(long,1);
      for it=1:long
...
for j=1:num
    if res(j).Area > 3000 %label cysts only if volume in time is higher than
    ll(h)=j;
    h=h+1;
    end
end
...
cleo=find(sup(ii,:)==0);
ss=size(cleo);
if ss(2)<3 %% AQUI IMPRIME EN FILE LOS CISTOS BUENOS
filename_of_responsive_cyst=strcat(strcat('11.final.time._scene_',int2str(iiii+1),'_mosaic_',int2str(iii+1),'_cyst_',int2str(ll(ii))),'.dat')
